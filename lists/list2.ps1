...
$notOnOriginalList = $installedPrograms | Where-Object { 
    $program = $_
    $matched = $false
    foreach ($originalProgram in $originalPrograms) {
        if ($originalProgram -like "Intel*Software Installer") {
            if ($program -like $originalProgram) {
                $matched = $true
                break
            }
        }
        else {
            if ($program -match [regex]::Escape($originalProgram)) {
                $matched = $true
                break
            }
        }
